...
int my_write(char* name) {
    ...
    int bnum;
    if(inode_num == -1){
        printf("write error: no such file\n");
        return -1;
    }
    Inode* inode = InodeAddr(inode_num);
    printf("write开始，进行P操作\n");
    ...
    if(inode->blockNum == 0) {
        ...
        bnum = inode->addrs[inode->blockNum - 1];
    }
    // printf("%d", bnum);
...
int my_read(char *name) {
    ...
    Inode* inode = InodeAddr(inode_num);
    printf("read开始，进行P(read)操作\n");
    sem_wait(&inode->sem_read);
    inode->readcnt++;
    if(inode->readcnt == 1){
        printf("read: 进行P(write)操作\n");
        sem_wait(&inode->sem_write);
    }
    printf("read开始，进行V(read)操作\n");
    ...
    printf("read结束，休眠5秒\n");
    sleep(5);//休眠5秒
    printf("read结束，进行P(read)操作\n");
    sem_wait(&inode->sem_read);
    inode->readcnt--;
    if(inode->readcnt == 0){
        printf("read: 进行V(write)操作\n");
        sem_post(&inode->sem_write);
    }
    printf("read结束，进行V(read)操作\n");
    sem_post(&inode->sem_read);
    return 0;
}
